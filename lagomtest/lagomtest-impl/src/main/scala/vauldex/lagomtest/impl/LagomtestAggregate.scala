...
/**
  * This provides an event sourced behavior. It has a state, [[LagomtestState]], which
  ...
  */
object LagomtestBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[LagomtestCommand]): Behavior[LagomtestCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, LagomtestEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[LagomtestCommand, LagomtestEvent, LagomtestState](
        persistenceId = persistenceId,
        emptyState = LagomtestState.initial,
...
/**
  ...
  */
case class LagomtestState(message: String, timestamp: String) {
  def applyCommand(cmd: LagomtestCommand): ReplyEffect[LagomtestEvent, LagomtestState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: LagomtestEvent): LagomtestState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[LagomtestEvent, LagomtestState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[LagomtestEvent, LagomtestState] =
    Effect
...
object LagomtestState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: LagomtestState = LagomtestState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[LagomtestCommand]("LagomtestAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[LagomtestState] = Json.format
}

/**
  * This interface defines all the events that the LagomtestAggregate supports.
  */
sealed trait LagomtestEvent extends AggregateEvent[LagomtestEvent] {
  def aggregateTag: AggregateEventTag[LagomtestEvent] = LagomtestEvent.Tag
}

object LagomtestEvent {
  val Tag: AggregateEventTag[LagomtestEvent] = AggregateEventTag[LagomtestEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends LagomtestEvent
...
/**
  ...
  */
trait LagomtestCommandSerializable

/**
  * This interface defines all the commands that the LagomtestAggregate supports.
  */
sealed trait LagomtestCommand
    extends LagomtestCommandSerializable
...
/**
  ...
  * application loader.
  */
object LagomtestSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[LagomtestState],
